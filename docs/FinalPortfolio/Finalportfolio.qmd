  ...
  vconcat = list(
    list(
      ...
      layer = list(
        ...
        list(
          mark = list(type = "text", dy = -10, fontSize = 10, fontStyle = "italic", opacity = 0.7),
          data = list(values = list(
            list(label_date = "2017-07-01", label_y = 65000, label = "2017 Bull Run"),
            list(label_date = "2018-07-01", label_y = 65000, label = "2018 Crash"),
            list(label_date = "2019-12-01", label_y = 65000, label = "Accumulation"),
            list(label_date = "2021-05-01", label_y = 65000, label = "2021 Bull Run"),
            list(label_date = "2022-06-01", label_y = 65000, label = "2022 Crash"),
            list(label_date = "2024-01-01", label_y = 115000, label = "Recovery")
          )),
          encoding = list(
            x = list(field = "label_date", type = "temporal", scale = list(domain = list(param = "brush"))),
            y = list(field = "label_y", type = "quantitative"),
...
The chart also supports interactive tooltips on hover, showing the exact date and price at any point along the line, which lets curious readers dig into specific moments without cluttering the static view.
...
The Bitcoin chart uses three layers stacked on top of each other: the colored background rectangles, the price line, and the text annotations. The twins chart from Exercise 1 also layers line marks connecting twin pairs with point marks at each end.
...
## Exercise 6
# a.
- Forcing a CSV field to numeric with a calculate transform

When I first loaded the Bitcoin CSV, the price line wasn't showing up at all even though the data looked fine in R. The problem was that Vega-Lite was reading the price column as a string when loading from a file. I had to add a calculate transform using toNumber(datum.price) to explicitly cast it to a number before it could be encoded on the y-axis. We never ran into this in class because we mostly used inline data or JSON files where types are already handled correctly.
